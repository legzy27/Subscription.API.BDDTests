...
When("I make an API request as a free user", () => {
  cy.request(`${BASE_URL}/free`).then((res) => {
    response = res;
  });
});

Then("I should receive a {int} OK response", (statusCode) => {
  expect(response.status).to.eq(parseInt(statusCode));
});

//Premium User Access Test
When("I request my playlists", () => {
  cy.request(`${BASE_URL}/premium`).then((res) => {
    response = res;
  });
});
...
Then("I should receive a HTTP {int} Bad Request response", () => {
